...
if [[ ! -s ~/.sbt ]]; then
  mkdir -p {{code_dir}}/.microbox/sbt_cache/sbt
  ln -s {{code_dir}}/.microbox/sbt_cache/sbt ~/.sbt
fi

# if ~/.ivy2 isn't a symlink, create it
if [[ ! -s ~/.ivy2 ]]; then
  mkdir -p {{code_dir}}/.microbox/sbt_cache/ivy2
  ln -s {{code_dir}}/.microbox/sbt_cache/ivy2 ~/.ivy2
fi
